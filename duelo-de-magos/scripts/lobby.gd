	...
	if network_type == LobbyManager.NetworkType.LAN:
		...
		if Steam and Steam.isSteamRunning():
			if Steam.getSteamID() > 0:
				...
				status_label.text = "Steam: No logueado D:"
		else:
...
func _on_server_created():
	status_label.text = "Servidor creado :D | Esperando jugadores..."
...
func _on_connection_ok():
	status_label.text = "Conectado al servidor :D"
	startBtn.visible = false
	_update_player_list()

func _on_connection_failed():
	status_label.text = "Error de conexión :c"
	hostBtn.disabled = false
	joinBtn.disabled = false
